...
class MainActivity : AppCompatActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.miLevels -> {
                showAlertDialog("Choose a level",findViewById(R.id.miLevels),View.OnClickListener {
                    //handle chose level
                })
            }
        }
    ...
    private fun initializeGame() {
        ...
        tvMoves.text = "${boardSize.toString().lowercase().capitalize(Locale.ROOT)}: ${boardSize.getNumLines()} x ${boardSize.getNumCol()}"
